...
- **Onboarding** (four screens; *explain first, then give reasons*). Screen one has to explain the product on its own: "Saw it in a video? Go there for real." Its illustration plays the whole loop, with a beach, a restaurant and a hidden beach, because Xplore is for any place a video shows (cafés, food spots, hidden spots), not only travel. A video plays, each place it shows is scanned and named, flies to a map as a numbered pin, the route draws, and it reads out *3 places found · Day 1 planned*. Then two reasons: the group vote (friends react one by one, Meera types a swap, the stop flips to Keeping) and weekends near home (a road draws from home to two spots; the district picker sits under it). Last, name and photo, shown as the invite a friend receives and filled in live as you type. Each illustration is one clock (0 → 1, 5.6–7.6s) that loops only while its page is on screen. Every element reads its own window of that clock (`src/components/onboarding/loop.ts`). Under Reduce Motion the clock rests on the finished frame. The illustration layer trails the swipe more than the words do (150 vs 36pt of parallax, plus a slight shrink and tilt), which gives the carousel its depth. Say *video*, never *reel*: not everyone knows the word, and YouTube doesn't use it.
...
- **Who's going** (first planning question: Just me · With my partner · With friends · With family) decides the rest. Solo: the share card just says "Share", and there's no vote. Partner: Arya votes, one bar slot, "You're both in". Friends: Riya, Kabir, Meera. Family: Amma, Appa and Ammu, shown as emoji faces (all three would otherwise be "A"), each with their own lines ("Is there parking?", "Reels spot, trust me"), "The family's in"; family also starts on a Relaxed pace. Sharing starts the vote, and it plays in the background (a runner at the root), so votes keep landing wherever you are. Confetti plays the first time you see an agreement, even one that finished while you were away.
- **Planning together, live** (with the Supabase backend connected): sharing creates the trip and sends a join link plus a six-letter code (no 0/O or 1/I). The first time, a name field sits above the share button. **Join** (Trips header, or the link) asks for the code and your name. The one who made the trip watches and locks it in; everyone else votes as themselves (no "who's voting" step), with a free-text note alongside the quick ones. Anyone can **Add a stop** in their own words: a title, an optional note, which day, morning/afternoon/evening and how long. It lands in its part of the day, pinned, borrowing the location of the stop before it, and waits for votes. Without the backend, all of this falls back to the scripted demo.
- **Trips tab** (Home · Trips · Map · Profile): every saved plan as a photo card with its dates, stop count, who's in (avatars) and a status chip: Planned (solo), Not shared yet, Voting · n/total with a thin accent bar filling live, the agreed line ("The family's in"), or ✓ Locked in. Tapping opens the plan, the share card or the vote. It's also the only screen that's about a friend who joined from a link. "Done" after sharing or voting lands here.
